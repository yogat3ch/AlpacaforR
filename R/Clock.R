...
#' @export
clock <- function(tz = Sys.getenv("TZ")){
  ...
  if (!grepl("New_York", .tz)) {
    cl$local <- lubridate::with_tz(cl$timestamp, tzone = .tz) |>
      {\(x) {
        rlang::list2(timestamp = x,
                     !!!purrr::map(cl[c("next_open", "next_close")], lubridate::with_tz, tzone = .tz),
             offset = lubridate::hours(cl$timestamp - lubridate::force_tz(x, "America/New_York")))
      }}()
