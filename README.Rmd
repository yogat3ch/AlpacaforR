...
The simplest way to set these values for this and future R sessions is to use `AlpacaforR::firstrun` to add these to the `.Renviron` file. If the `.Renviron` file does not exist it will be created in the R root folder `path.expand("~")`. 

`firstrun` has arguments: `paper_api`, `live_api`, `polygon_api`, `pro` and `live`. `paper_api` and `live_api` are named vectors with key & secret for paper and live accounts respectively, while `polygon_api` is the secret key for a Polygon account. The `pro` argument specifies whether an Alpaca Pro subscription is available and `live` argument is a logical which sets the default value for `live` in future sessions. See [Live or Paper](#live-or-paper) and \code{\link[AlpacaforR]{firstrun}} for details. 
...
Add FB, AMZN, NFLX, GOOG and update the watchlist name to FAANG. TThe default for `action` when a `new_name` is specified is to `a`dd new symbols when changing the name. Similarly, if just a `new_name` is provided, the existing `symbols` will be preserved.
```{r}
...
Individual assets can be deleted or added to watchlists using `action = "add"` or `"delete"` (`"a"`/`"d"` for short).
...
Data is returned as a list of `tsymble`s (one for each symbol provided to `ticker`) in OHLCV format 📊. The Polygon API also returns `n` & `vw` (See the [endpoint docs for details]([Polygon's Aggregates Endpoint](https://polygon.io/docs/#get_v2_aggs_ticker__ticker__range__multiplier___timespan___from___to__anchor)). 
...
The v2 & Polygon APIs do not have exclusive date bound options, if these arguments are used for these APIs they are considered `from`/`to` inclusive when sent to the API. 
...
The v2 API offers the following additional functionality:
*Hourly bars*
The v2 API has a 1 hour `timeframe = "h", multiplier = 1` timeframe available when fetching bars.
*Additional quote & trade endpoints*
The V2 API has two additional quote & trade endpoints that retrieve quote and trade data for a given time period. *WARNING* These endpoints return an enormous amount of data for each day. For example: a request spanning a single day (ie 5/26-5/27) can take ~ 3m to retrieve.
Use `timeframe`:
...
*Note*
Free accounts for the Polygon API are limited to five requests per minute. If the rate limit is reached, a cooldown timer of 60s will be triggered before the next query is sent. 
