...
## `write_dir`: Logging to disk

When a socket is instantiated with `write_dir = 'AlpacaStreams'` (the default), a folder is created called _AlpacaStreams_ with sub-directories labeled synonymous to each active socket's url that contain log files for each channel. 

```{r 'Log files present'}
list.dirs("AlpacaStreams", recursive = TRUE)
(files <- list.files("AlpacaStreams", recursive = TRUE, full.names = TRUE))
```
...
`msg` refers to any message received over the websocket.
```{r 'expression'}
...
Let's set the notification tracker on the by minute stream for BYND
```{r 'am.gme'}
AS$channel("BYND", msg_action = msg_action)
```
...
When ready to close sockets, individual sockets can be closed by calling close on the specific socket:
```{r 'close_poly'}
AS$Alpaca$data$close()
```

Or call `$close` from the `AlpacaStreams` object to close all sockets:
```{r 'close_all'}
AS$close()
```

If you have ideas for how the interface might be improved or you encounter a bug, please open an issue on [Github](https://github.com/yogat3ch/AlpacaforR)
