...
### 0.7.0 2021-03-16 Data API v2 Support - Polygon & Alpaca Separated
 - Polygon is no longer supplying data for Alpaca users and is now a separate service. 
 - `POLYGON-KEY` is an environmental variable that must be set to the Polygon API key Polygon related functionality to work. 
 - The version `v` argument to `market_data` now accepts `"p"/"polygon"` to access the [Aggregates endpoint](https://polygon.io/docs/get_v2_aggs_ticker__stocksTicker__prev_anchor) from polygon.
 - The `polygon` function has been modified to continue working properly.
 - The `AlpacaStreams` Polygon websocket functionality has been modified to accommodate the change, however, changes are untested.
 - Polygon related endpoints will no longer be actively maintained hereforward (unless I receive many requests for it to be maintained & sufficient donations to maintain a subscription!).

#### 0.6.0 2021-02-08 Re-write of websockets functionality using {R6}
 - The `{websocket}` package uses `{R6}` natively. AlpacaforR relies on `{websocket}` for interfacing with the server, but S3 type functions were previously used to establish this interface resulting in a somewhat confusing experience with manipulating the sockets. The new `AlpacaStreams` `R6` object provides a single interface for interacting with websocket connections that simplifies common tasks.
 
#### 0.5.0 2020-12-18 Optimization of `market_data` using new functions from the {tsibble} package
 - The `market_data` function is now significantly faster and more reliable in building complete datasets per the user's request in spite of API limits on data returned per call. Repeat requests are made to the API until the dataset is as complete as it can be. 
 
#### 0.4.4 2020-11-12 Support for trailing stops
 - Now supports trailing stop orders recently released by Alpaca.
 
#### 0.4.0 2020-05-24 Package Overhaul
...
The current root folder of this R instance is: `r path.expand("~")`, the `.Renviron` file will reside there if it doesn't already. 

`firstrun` allows for setting the following environmental variables:
 - `paper_api`: the Alpaca paper API key and secret 
   - `APCA-PAPER-KEY`
   - `APCA-PAPER-SECRET`
 - `live_api`: the Alpaca live API key and secret
   - `APCA-LIVE-KEY`
   - `APCA-LIVE-SECRET`
 - `polygon_api`: the Polygon API key
   - `POLYGON-KEY`
 - `live`: default option for which API to use live `TRUE` / paper `FALSE`
   - `APCA-LIVE`
 - `pro`: whether or not a pro `TRUE` data subscription or a basic `FALSE` is used for the Alpaca Data API v2
   - `APCA-PRO`


```{r 'first_run', eval = FALSE}
firstrun(
  paper_api = c(key = "Your-paper-key", secret = "Your-paper-secret"),
  live_api = c(key = "Your-live-key", secret = "Your-live-secret"),
  polygon_api = c(key = "Your-polygon-key"),
  live = FALSE,
  pro = FALSE
)
```

Alternatively, if using RStudio, these parameters can be added to the `.Renviron` file by typing `usethis::edit_r_environ()` at the console. The keys are added as `name = key` pairs like so:
...
```
If `usethis` is not installed, it can be installed from CRAN, or the .REnviron file can be edited manually with the following guide [set environment variables permanently](https://stackoverflow.com/questions/49738564/r-set-environment-variable-permanently).

Test that all environment variables have been properly set by calling:
```r
...
Sys.setenv('APCA-LIVE-KEY' = "...")
Sys.setenv('APCA-LIVE-SECRET' = "...")
...
Not all functions require this since some functions use the same URL regardless of the account type. These functions are `assets` 💰, `calendar` 🗓, `clock` ⏰, and `market_data` 📊 .


# Package Functionality
The functionality in the `AlpacaforR` package maps neatly onto the endpoints listed in the [API version 2 Documentation](https://alpaca.markets/docs/api-documentation/api-v2/) for ease of reference. For any function use `?function_name` at the console to view the function's documentation which will provide a great deal more depth of detail regarding it's arguments and what the function returns.
...
### `account_activities`
`account_activities` accesses the [Account Activities Endpoint](https://alpaca.markets/docs/api-documentation/api-v2/account-activities/) to view all account activities, optionally filtered by type and date range. This endpoint supports paging - advance pages by providing the last ID supplied for a given page to `page_token`.
...
### `account_portfolio`
`account_portfolio` accesses the [Portfolio History Endpoint](https://alpaca.markets/docs/api-documentation/api-v2/portfolio-history/) and returns the data covering equity and profit loss information of the account for a period of time aggregated by a specified timeframe (*optional*) or up to a specific end date (*optional*). 
...
The [Assets Endpoint](https://alpaca.markets/docs/api-documentation/api-v2/assets/) serves a master list of assets 💰 available for trade and data consumption from Alpaca. Assets are sorted by asset class, exchange and symbol.
...
```{r 'wl delete test', include = FALSE}
...
```{r 'create wl'}
(wl <- watchlist("test", symbols = "MSFT", action = "c"))
```
...
```{r retrieve_test}
(test <- watchlist("test"))
# attributes must be nullified because the timestamp will not match
identical(
  rlang::set_attrs(test, NULL),
  rlang::set_attrs(wl, NULL)
)
```

Each watchlist `tibble` has an `info` attribute that stores details about when it was created, last updated and more. *Note* these attributes are not formalized as slots in a new class so they will be lost when the object is transformed. 
...
Watchlist items can be replaced by changing the action to `"replace"` or `"r"` for short. *Note* that the default behavior when using `action = "replace"` is to replace the existing assets in the watchlist with those specified to `symbols` in the call to replace.
Any watchlist indicated with the `watchlist_id` will automatically be renamed when a `new_name` is provided.

```{r rename_test}
# Update the watchlist name to test2 and replace the symbols with "FB", "AMZN", "NFLX", "GOOG"
(wl <- watchlist("test", new_name = "test2", symbols = c("FB", "AMZN", "NFLX", "GOOG", "WMT"), action = "r"))
...
Individual assets can be deleted or added to watchlists using actions `"add"` or `"delete"` (`"a"`/`"d"` for short). Here, the watchlist name is updated to better reflect the symbols it will contain after the call is completed.

```{r 'wl delete WMT'}
# Walmart needn't be in there, remove it and rename the watchlist appropriately
(wl <- watchlist("test2", new_name = "FANG", symbol = "WMT", action = "d"))
#>                                     id     class exchange symbol
#> 1 fc6a5dcd-4a70-4b8d-b64f-d83a6dae9ba4 us_equity   NASDAQ     FB
#> 2 f801f835-bfe6-4a9d-a6b1-ccbb84bfd75f us_equity   NASDAQ   AMZN
#> 3 bb2a26c0-4c77-4801-8afc-82e8142ac7b8 us_equity   NASDAQ   NFLX
...
(wl <- watchlist("FANG", new_name = "FABANGG", symbol = c("AAPL", "BYND", "GOOGL"), action = "a"))
#>                                     id     class exchange symbol
...
Data is returned as a list of `tibble`s (one for each symbol provided to `symbol`) with OHLCV bar format 📊 in each. The only required input is the symbol(s) as a character vector, and it will return pricing data for the last 7 trading days, by day, for each symbol. In the call below, `v = 1` is the default, thus the data returned is from the Alpaca API: 
...
```{r 'market_Data amzn after until'}
...
Here, data for multiple symbols is retrieved at a five minute level of `timeframe` aggregation. The results would be quite long, so just the range of time for each day is shown:

```{r 'market_data bynd fb'}
symbols <- c("BYND", "FB")
#Getting 5Min bar pricing data for the last 6 trading days. 
market_data(symbols, m = 5, timeframe = "Min", from = Sys.Date() - lubridate::weeks(1)) %>% 
  purrr::map(~{
    .dat <- .x
    .x %>% 
...
```{r 'market_data bynd'}
bynd <- market_data("BYND", v = "p", time = "m", m = 5)
#> `from` argument omitted, setting to 2020-05-22
#> `to` argument omitted, setting to 2020-05-29
#> Floor/Ceiling dates are necessary to retrieve inclusive aggregates
...
```{r 'market_data minute'}
(md <- market_data("BYND", v = "p", time = "m", m = 1, from = d, to = d))
#> Floor/Ceiling dates are necessary to retrieve inclusive aggregates
...
```{r 'market_data trading hours diff'}
...
```{r 'market_data nontrading hours'}
...
```{r 'market_data polygon hours'}
market_data("BYND", v = "p", time = "h", m = 1, from = "2020-05-01", to = "2020-05-01")
```
...
```{r 'market_data polygon days'}
market_data("BYND", v = "p", time = "d", m = 1, from = lubridate::today() - lubridate::weeks(1))
```
...
```{r 'market_data polygon weeks'}
market_data("BYND", v = "p", time = "w", m = 1, from = lubridate::floor_date(lubridate::today(), "quarter"))
```
...
```{r 'sleep1', include = FALSE}
Sys.sleep(60)
```


```{r 'market_data polygon months'}
market_data("BYND", v = "p", time = "M", m = 1, lubridate::floor_date(lubridate::today(), "year"))
```
...
```{r 'market_data polygon quarters'}
market_data("BYND", v = "p", time = "q", m = 1, from = lubridate::floor_date(lubridate::today(), "year"))
```
...
```{r 'market_data polygon year'}
market_data("FB", v = "p", time = "y", m = 1, from = lubridate::floor_date(lubridate::today(), "year") - lubridate::years(4))
```
...
Getting, submitting, and canceling 🚫 orders are also made easy through `orders()` and `order_submit()`, but require some specific arguments. Visit the [Orders API](https://alpaca.markets/docs/api-documentation/api-v2/orders/) webpage to learn everything there is to know about the requests and responses for this API.
...
use `order_submit()` with the appropriate arguments and fire away 🚀. The value supplied to `action` determines what type of action will be taken and what parameters are required. The default action is `"submit"`.
...
(bo <- order_submit("bynd", side = "b", qty = 2, type = "m"))
}
#> # A tibble: 1 x 28
#>   id    client_order_id created_at          updated_at         
#>   <chr> <chr>           <dttm>              <dttm>             
...
`order_submit` has extensive built-in auto-assumption of omitted arguments where they can be assumed based on other provided arguments. The documentation (`?order_submit`) has examples that go into detail as to the parameters necessary to invoke accurate auto-assumption for each `action`. Since traders will often place a stop, limit, or stop limit order following a buy order to mitigate downside risk, an 'expedited sell'  is one such auto-assumption feature. To execute an 'expedited sell', the function needs only the Order ID of or `tbl` returned from placing a buy order and the price of the stop/limit/stop limit. The `side` will be assumed as sell, the `qty` will be assumed to be the full quantity of the buy order, the `type` will be assumed from the arguments provided (`stop`, `limit` or both) and the `client_order_id` for the sell order will be the buy order id to conceptually link the two orders.

##### Set the `stop`/`limit` or both

To do so it's necessary to know the current price of the stock. This can be retrieved by calling `orders` to see what price the buy order filled at, or by using the [Last Quote endpoint](https://alpaca.markets/docs/api-documentation/api-v2/market-data/alpaca-data-api-v1/last-quote/).
...
```
With the price, a stop order can be placed $5%$ lower than what it was bought at. To connect this sell order to the buy order for cost basis reporting purposes, set `client_order_id = T` and the `client_order_id` for this sell order will be set to the Order ID of the buy order.

```{r 'order client_id'}
if (.open) {
  (so <- order_submit(bo$id, stop = lq$price * .95, client_order_id = T))
}
#> `side` set to 'sell'
#> `qty` set to 2
...
Informative messages indicate where the function made assumptions about the values for unspecified arguments. 

To extend the example, suppose the price of BYND went up since the order was first placed, yet the stop is still set at 5% lower from where the order was bought. It would be wise to move the stop price up a bit to follow the price. This can be done for simple orders (of which this is one), with `action = 'replace'`. 

##### The `client_order_id` argument

The replacement order will have a field `replaces` that will indicate the order it replaced, which is, in this case, the previous sell order. The sell order placed above was linked to the buy order via the `client_order_id` such that cost basis can be determined with ease later. However, `client_order_id` must be unique for each order. So how does one keep this replacement order connected to the original buy order? 

The simplest way to do so is to provide a custom `client_order_id` with an increment suffix appended for each successive replacement order. The full length of the `client_order_id` must be under `48` characters and the Alpaca generated IDs are `36` characters, which leaves $48 - 36 = 12$ characters for the suffix. 
...
if (.open && isTRUE(nrow(so) > 0)) {
  (ro <- order_submit(so$id, action = "r", stop = lq$price * .96, client_order_id = client_order_id))
}
#> # A tibble: 1 x 28
#>   id    client_order_id created_at          updated_at         
#>   <chr> <chr>           <dttm>              <dttm>             
...
```{r 'order tp'}
take_profit <- list(l = lq$price * 1.05)
```

and a lower limit, stop or both specified to `stop_loss` as a named list with the names `'stop_price'/'s'` & `'limit_price'/'l'`:

```{r 'order sl'}
stop_loss <- list(s = lq$price * .95)
```


Now the `oco` order class can be placed  by providing the id of the original buy order to passively set argument defaults, while with another increment to the `client_order_id` can link this order to that original buy order.
...
All current positions or only the positions specified by `symbol` are retrieved by calling `positions()`. See `?positions` for more details. `positions` has multiple actions:

 - `"get"/"g"` positions (**Default**)
 - `"close"/"c"` a position or positions provided by `symbol`
 - `"close_all"` which will close all open orders on currently held positions and then close those positions by selling all shares. Think of `action = "close_all"` as an emergency kill switch that will liquidate all positions. 
...
## Polygon
Polygon no longer provides API access to Alpaca live trading account users. However, the `polygon` function may be used to access endpoints allowed by specific Polygon subscription. See [Pricing](https://polygon.io/pricing) for details. [Reference](https://polygon.io/docs/#Reference) and some [Stocks/Equities](https://polygon.io/docs/#Stocks___Equities) endpoints on the Polygon API. `AlpacaforR` provides a single go-to function to access all of the available Polygon endpoints: `polygon`. This function takes as it's first argument `ep`, short for endpoint, which can be the full name of the endpoint as it appears in the [Docs](https://polygon.io/docs/) or a one to two letter abbreviation of the endpoint which is typically the first letter of each of the first two words (that aren't wrapped in parentheses) of the name of the endpoint. The one exception being Snapshot - Single Ticker (`st`), which would otherwise conflict with Stock Splits (`ss`). 
...
```{r 'polygon hq request', eval = FALSE}
...
```{r 'polygon last quote', eval = FALSE}
