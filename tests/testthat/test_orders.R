...
vcr::use_cassette("market open?", {
  .open <<- clock()$is_open
})
vcr::use_cassette("orders", {
  .ao <- orders()
})

if (isTRUE(inherits(.ao, "tbl") && try(nrow(.ao) > 0))) {
  vcr::use_cassette("pre cancel all orders", {
    order_submit(a = "cancel_all")
  })
  
}
vcr::use_cassette("empty orders", {
  .ao <- orders()
})

if (.open) {
vcr::use_cassette("orders returns no orders properly.", {
...
Check `ticker_id` or set status = 'all' to see all orders."),"list")
  ...
  vcr::use_cassette("if there are orders to cancel", {
    test_that("if there are orders to cancel", {
...
vcr::use_cassette("order_submit errors if type = limit and limit is not set", {
...
vcr::use_cassette("order_submit errors if type = stop and stop is not set", {
...
vcr::use_cassette("order_submit errors if type = stop_limit and stop is not set", {
...
vcr::use_cassette("order_submit errors if type = stop_limit and limit is not set", {
...
vcr::use_cassette("order_submit errors if qty is not set", {
...
vcr::use_cassette("order_submit properly places a simple buy order", {
...
vcr::use_cassette("orders properly detects the order", {
...
vcr::use_cassette("last quote", {
  .lq <<- polygon("lq", symbol = "AMZN")
})


if (.open) {
vcr::use_cassette("An expedited stop can be placed on the order with appropriate messages", {
...
vcr::use_cassette("An expedited stop warns if the buy order did not fill and there is an insufficient quantity available", {
  ...
  vcr::use_cassette("Expedited stop warns when market is not open", {
    test_that("Expedited stop warns when market is not open", {
...
if (.open && exists(".so")) {
vcr::use_cassette("order_submit properly modifies the simple sell order", {
...
vcr::use_cassette("order_submit errors if outside of market hours", {
...
vcr::use_cassette("order_submit properly places a bracket order_class", {
...
vcr::use_cassette("order_submit properly cancels the bracket order", {
...
if (.open) {
vcr::use_cassette("order_submit properly cancels the simple sell replacement order", {
...
if (.open) {
vcr::use_cassette("order_submit properly places an oco order_class", {
...
vcr::use_cassette("order_submit warns appropriately when market is closed", {
...
vcr::use_cassette("order_submit properly places an oto order_class", {
...
vcr::use_cassette("retrieve orders for cancel all", {
  .oo <- orders()
})
vcr::use_cassette("cancel all orders again", {
  .ca <- order_submit(a = "cancel_all")
})
if (.open && length(.oo) > 0) {
vcr::use_cassette("order_submit properly cancels all open orders when market is open and there are orders to cancel", {
...
vcr::use_cassette("order_submit properly cancels all oepn order when there are no open orders to cancel", {
